...
<style>
    ...
    button { width: 100%; padding: 15px; border: none; border-radius: 8px; font-weight: bold; cursor: pointer; transition: 0.3s; }
    .btn-reg { background: var(--success); color: white; }
    .btn-adm { background: var(--primary); color: white; }
    .btn-reg:hover, .btn-adm:hover { opacity: 0.8; transform: scale(1.02); }
    table { width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 14px; }
    th { background: #334155; padding: 12px; text-align: left; }
    td { padding: 12px; border-bottom: 1px solid #334155; }
    .faturamento { font-size: 32px; color: var(--success); font-weight: 800; margin: 10px 0; }
</style>
"""
...
async def index():
    return f"""{CSS}
    <div class="container">
        <h2 style="text-align:center;">🚀 AutoReport <span style="color:var(--primary)">Business</span></h2>
        <p style="text-align:center; color:#94a3b8;">Registo de Vendas em Tempo Real</p>
        <form action="/registrar_venda" method="post">
            <label>Chave do Trabalhador</label><input name="c" type="password" placeholder="••••" required>
            <label>Nome do Produto</label><input name="p" type="text" placeholder="Ex: Sumo de Maçã" required>
            <div style="display:flex; gap:10px;">
                <div style="flex:1;"><label>Qtd</label><input name="q" type="number" step="0.01" value="1"></div>
                <div style="flex:1;"><label>Preço (MT)</label><input name="pr" type="number" step="0.01" placeholder="0.00"></div>
            </div>
            <button type="submit" class="btn-reg">CONFIRMAR REGISTO ✅</button>
        </form>
    </div>"""
...
async def registrar(c:str=Form(...), p:str=Form(...), q:float=Form(...), pr:float=Form(...)):
    ...
    return "✅ Sucesso! <a href='/' style='color:white;'>Voltar</a>"
...
async def admin_view():
    ...
    <div class="container">
        ...
        <form action="/admin/dash" method="post">
            <input name="g" type="email" placeholder="Gmail do Dono" required>
            <input name="s" type="password" placeholder="Senha de Acesso" required>
            <button type="submit" class="btn-adm">ENTRAR NO DASHBOARD</button>
        </form>
    </div>"""
...
async def dash(g:str=Form(...), s:str=Form(...)):
    ...
    if not loja or not pwd_context.verify(s, loja['senha_hash']): return "❌ Erro de Acesso"
    ...
    total_geral = sum(v['quantidade'] * v['preco'] for v in vendas)
    linhas = "".join([f"<tr><td>{v['data_venda'].strftime('%H:%M')}</td><td>{v['produto']}</td><td>{v['quantidade']*v['preco']:.2f}</td></tr>" for v in vendas])

    return f"""{CSS}
    <div class="container" style="max-width:800px;">
        <p style="color:#94a3b8; margin:0;">Dashboard de {loja['nome']}</p>
        <div class="faturamento">{total_geral:.2f} MT</div>
        <form action="/admin/export" method="post">
            <input type="hidden" name="id" value="{loja['id']}">
            <button type="submit" style="background:#334155; color:white; margin-bottom:20px;">📥 DESCARREGAR RELATÓRIO EXCEL</button>
        </form>
        <table>
            <thead><tr><th>Hora</th><th>Produto</th><th>Total (MT)</th></tr></thead>
            <tbody>{linhas}</tbody>
        </table>
    </div>"""
...
async def export(id:int=Form(...)):
    ...
    return StreamingResponse(output, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", 
                             headers={{"Content-Disposition": f"attachment; filename=Vendas_{datetime.now().strftime('%d_%m')}.xlsx"}})

@app.get("/setup_sistema", response_class=HTMLResponse)
async def setup_v(): 
    return f"""{CSS}<div class="container"><form action="/setup_sistema" method="post">
    <h2>🏗️ Setup</h2>
    <input name="mestre" placeholder="Chave Mestre" type="password">
    <input name="nome" placeholder="Nome da Loja">
    <input name="gmail" placeholder="Gmail">
    <input name="senha" placeholder="Senha" type="password">
    <input name="chave_t" placeholder="Chave Trabalhador">
    <button type="submit" class="btn-adm">CRIAR SISTEMA</button>
    </form></div>"""
